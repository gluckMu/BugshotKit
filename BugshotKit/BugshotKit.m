...
// ADD BY Lili
// 崩溃收集
void handleCrash(NSString *crashInfo, NSString *type)
{
    NSString *logInfo = [[BugshotKit sharedManager] logStringWithDateStamps:YES];
    ...
    NSString *string = [NSString stringWithFormat:@"appName: %@\nappVersion: %@\nsystemVersion: %@\ndeviceModel: %@\n=============异常崩溃报告=============\n%@\n=============崩溃前日志=============\n%@", appNameString, appVersionString, UIDevice.currentDevice.systemVersion, modelIdentifier, crashInfo, logInfo];
    ...
    NSString *dir = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"FMCrash"];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    BOOL isDir;
    if (!([fileManager fileExistsAtPath:dir isDirectory:&isDir] && isDir)) {
        [fileManager createDirectoryAtPath:dir withIntermediateDirectories:YES attributes:nil error:nil];
    }
    NSString *path = [dir stringByAppendingPathComponent:[NSString stringWithFormat:@"Exception-%@-%@", dateString, type]];
...
void UncaughtExceptionHandler(NSException *exception) {
    ...
    handleCrash(crashInfo, @"uncaught");
    if (g_vaildUncaughtExceptionHandler) {
        NSLog(@"BugshotKit will invoke registed uncaught exception handler");
...
void InstallUncaughtException() {
    ...
    if (g_vaildUncaughtExceptionHandler) {
        NSLog(@"BugshotKit find that someone has registed uncaught exception handler");
    }
...
void SignalExceptionHandler(int signal) {
    ...
    handleCrash(mstr, @"signal");
}

// 捕获Signal
void InstallSignalException() {
...
- (instancetype)init
{
    if ( (self = [super init]) ) {
        ...
        // iOS 10以上ASL被新的日志API os_log替换 但是os_log中并没有提供asl_search同样的功能
        // 所以在iOS 10以上暂时使用重定向来实现 这会导致日志不会在debug console中打印
#ifdef __IPHONE_10_0
...
{
    dispatch_async(self.logQueue, ^{
        NSString *log = [self logStringWithDateStamps:dateStamps];
...
- (NSString *)logStringWithDateStamps:(BOOL)dateStamps {
